     ...
     while($datos = fgetcsv ($fp, 1000, "\t")){
		  ...
		  if (pg_num_rows($datostemp)>0) {
		   ...
		   <?php } else { 
							 ...
							 $strqueryd="INSERT INTO laboratorios (id_lab,id_dep,	
							                                       id_responsable,nombre,dir_postal,	
							                                       act_generales,carreras,asignaturas,
																   id_edif, detalle_ub,capacidad,tipo_lab)
                                          VALUES (%d,%d,
										          %d,'%s','%s',
				                                 %d,'%s','%s',
												  %d,'%s','%s','%s')";
				   
                 $queryid=sprintf($strqueryd, $datosdec[0],$datosdec[1],
				                              $datosdec[2],$datosdec[3],$datosdec[4],
											  $datosdec[5],$datosdec[6],$datosdec[7],
											  $datosdec[8],$datosdec[9],$datosdec[10],
											  $datosdec[11]);
