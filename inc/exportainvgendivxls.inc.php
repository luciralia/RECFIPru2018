...
$nombdiv= pg_fetch_array($registrodiv);
    ...
    $texto='Content-Disposition: attachment;filename="inventariodiv_' . date("Ymd-His") . "_" . $nombdiv[0]. '.xls"';


 }elseif  ($_SESSION['tipo_usuario']==10 && $_SESSION['id_div']=='' ) 
   $texto='Content-Disposition: attachment;filename="inventariodiv' . date("Ymd-His") . "_FacultadIngenieria.xls";
    elseif  ($_SESSION['tipo_usuario']==10 && $_SESSION['id_div']!='' ) 
	{
		$querydiv="SELECT nombre FROM divisiones
           WHERE id_div=" . $_SESSION['id_div'] ;
