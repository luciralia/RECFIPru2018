...
$titulo='Importar Área Patrimonio ';
 ...
 if($size > 0){
     ...
     while($datos = fgetcsv ($fp, 1000, "\t")){
		...
		// $guardaPatrimonio->patrimonio($datosdec);
         ...
         $querytemp="SELECT * FROM area_patrimonio WHERE 
		              id_area_pat="."'".$datosdec[0]."'";
		 ...
		 if ($importado>0) {   ?>      
               <legend align="left"> <strong><?php  echo "Se importó previamente el área con identificador " . $datosdec[0]; ?></strong></legend> 
    <?php
		 }else {
   
   	  $queryd="INSERT INTO area_patrimonio (id_area_pat,id_edif_pat,nivel_pat,
	                                        division_pat,local_pat)
		      ...
		      VALUES ('%s','%s','%s',
                ...
                $result=pg_query($queryid) or die('ERROR AL INSERTAR EN Área Patrimonial ' . pg_last_error());
